...
void hbp(DeviceImage<PIXEL_COST> &cost_map, DeviceImage<float> &depth)
{
	...
	const int hbp_level = 3;
	int hbp_iterate[5] = {5, 10, 10, 0, 0}; // from fine to coarse 3 level
...
//   float P2 = 1.5f;
  ...
  if(depth_id < DEPTH_NUM - 1)
  	min_cost = fminf(min_cost, neighbor_cost[dir][depth_id + 1] + P1);
  // if(depth_id > 1)
  // 	min_cost = fminf(min_cost, neighbor_cost[dir][depth_id - 2] + 2.0f * P1);
  // if(depth_id < DEPTH_NUM - 2)
  // 	min_cost = fminf(min_cost, neighbor_cost[dir][depth_id + 2] + 2.0f * P1);
