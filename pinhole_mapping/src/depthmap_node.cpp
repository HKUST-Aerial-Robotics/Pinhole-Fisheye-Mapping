...
bool rmd::DepthmapNode::init()
{
  ...
  depthmap_ = std::make_shared<rmd::Depthmap>(resize_width, resize_height, resize_fx, resize_cx, resize_fy, resize_cy, undist_map1, undist_map2);

  bool pub_pointcloud = false;
  nh_.getParam("publish_pointcloud", pub_pointcloud);
  publisher_.reset(new rmd::Publisher(nh_, depthmap_, pub_pointcloud));
    ...
    const geometry_msgs::PoseStampedConstPtr &pose_input)
{
  ...
  printf("depthmap cost %f ms (combine the remap image and cuda).\n", (std::clock() - start) / (double)CLOCKS_PER_SEC * 1000);
  start = std::clock();
  if (has_result)
    PublishResults();
  printf("publish image cost %f ms.\n", (std::clock() - start) / (double)CLOCKS_PER_SEC * 1000);
  printf("total call back cost %f ms.\n", (std::clock() - start2) / (double)CLOCKS_PER_SEC * 1000);
}

void rmd::DepthmapNode::PublishResults()
{
